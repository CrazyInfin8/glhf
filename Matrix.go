...
func NewMatrixFromTranformable(t Transformable) Matrix {
	p := t.Position()
	s := t.ScaleVector()
	return NewMatrixFromBox(p.X, p.Y, t.Angle(), s.X, s.Y)
}
...
func fromCPTransform(cpt cp.Transform) Matrix {
	t := (*struct {
...
func (m *Matrix) toCPTransform() cp.Transform {
	return cp.NewTransform(
		m.A, m.C, m.TX,
		m.B, m.D, m.TY,
	)
}
